...
export default function DoctorPatientPrescriptionsTab({
  ...
  const [mode, setMode] = useState(routine.length || existing.length ? 'routine' : 'add')
  ...
  const routineRows = useMemo(() => {
    const merged = [
      ...routine,
      ...existing.filter((item) => !routine.some((row) => row.id === item.id || row.name === item.name)),
    ]
    ...
    const medicines = catalog
      ...
      .map((item) => {
        ...
        return {
          ...
          instructions: `Add ${item.name} to your medicine routine as per doctor: ${frequency}, for ${duration}.`,
        }
      })
  ...
  return (
      ...
      action={
        mode === 'add' ? (
          ...
          >
            ...
            {selectedIds.length
              ? `Add to routine (${selectedIds.length})`
              : 'Add to routine'}
          </button>
        ) : null
      }
    >
      <RxModeTabs
        value={mode}
        counts={{ add: catalog.length, routine: routineRows.length }}
        onChange={setMode}
      />
      ...
      <label className="shrink-0 mx-3 mt-2 mb-2 flex items-center gap-2.5 rounded-xl bg-[#F4F7FA] border border-[#E6EBF1] px-3 min-h-11">
        ...
        <input
          ...
          placeholder={
            mode === 'routine'
              ? 'Search patient routine'
              : 'Search medicine (Dolo, Cetirizine…)'
          }
      ...
      {mode === 'add' ? (
        <div className="shrink-0 mx-3 mb-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
      ...
      {mode === 'add' && selectedItems.length ? (
        <div className="shrink-0 mx-3 mb-2 rounded-xl border border-teal/20 bg-[#E8F7F6] px-3 py-2.5">
          ...
          <p className="text-[12px] text-navy mt-2">
            Patient will take these as: <span className="font-semibold">{frequency}</span> ·{' '}
        ...
        !addRows.length ? (
          ...
          <PatientChartTable fill>
            <thead className="bg-[#E8F7F6] sticky top-0 z-10">
              <tr>
                {['', 'No.', 'Medicine', 'Dose', 'Use for', 'Status'].map((label, index) => (
                  ...
                  >
                    <PatientChartTd center>
                      <input
                        type="checkbox"
                        checked={checked}
                        disabled={item.alreadyInRoutine}
      ...
      {mode === 'routine' ? (
        !routineRows.length ? (
          <PatientChartEmpty text="No medicines in this patient’s routine yet. Add medicines as the doctor advised." />
        ) : (
          <PatientChartTable fill>
            <thead className="bg-[#E8F7F6] sticky top-0 z-10">
              <tr>
                {['No.', 'Medicine', 'Dose', 'Schedule', 'Duration', 'Type'].map((label, index) => (
