...
export default function DoctorNextVisitPanel({ visit, onOpen, onAccept }) {
  if (!visit) {
    return (
      <section className="bg-white rounded-2xl border border-border-gray shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-5 sm:p-6 flex flex-col items-start justify-center gap-3 h-full">
        <IconTile>
  ...
  return (
    <section className="bg-white rounded-2xl border border-border-gray shadow-[0_8px_24px_rgba(15,23,42,0.06)] p-5 flex flex-col gap-4 h-full">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-lg sm:text-xl font-bold text-navy leading-tight">Next patient</h2>
            <p className="text-sm text-body-gray mt-0.5">Your upcoming appointment</p>
          </div>
        </div>
        <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold shrink-0 ${statusTone}`}>
      ...
      <div className="flex items-center gap-3.5">
        <div className="w-14 h-14 rounded-full overflow-hidden shrink-0 bg-teal-light">
          <img src={card.patientPhoto} alt={card.patientName} className="w-full h-full object-cover object-top" />
        </div>
        <div className="min-w-0">
          <p className="text-lg sm:text-xl font-bold text-navy leading-tight truncate">{card.patientName}</p>
          <p className="text-sm text-body-gray mt-1 flex flex-wrap items-center gap-x-2 gap-y-1">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 rounded-2xl bg-[#F4F7FA] overflow-hidden">
        {details.map((item, index) => {
          const Icon = item.icon
          const borders = [
            '',
            'border-l border-[#E4EAF0]',
            'border-t lg:border-t-0 lg:border-l border-[#E4EAF0]',
            'border-t border-l lg:border-t-0 border-[#E4EAF0]',
          ]
          return (
            <div key={item.label} className={`flex items-start gap-2.5 p-3 sm:p-3.5 ${borders[index]}`}>
              <IconTile>
                <Icon className="w-4 h-4" strokeWidth={1.75} />
              </IconTile>
              <div className="min-w-0">
                <p className="text-[11px] text-body-gray">{item.label}</p>
                <p className="text-sm font-bold text-navy mt-0.5 leading-tight truncate">{item.value}</p>
                <p className="text-[11px] text-body-gray mt-0.5 truncate">{item.hint}</p>
              </div>
            </div>
          )
        })}
      ...
      <div className="rounded-2xl bg-[#E7F6F5] px-3.5 py-3 flex items-start sm:items-center gap-3">
        <IconTile>
          <ClipboardList className="w-4 h-4" strokeWidth={1.75} />
        </IconTile>
        <p className="text-sm font-bold text-teal shrink-0">Visit reason</p>
        <p className="text-sm text-navy leading-relaxed min-w-0">{card.prepNote}</p>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm text-body-gray pt-1 border-t border-dashed border-border-gray">
        <span className="inline-flex items-center gap-2 min-w-0">
          <MapPin className="w-4 h-4 text-teal shrink-0" />
          <span className="truncate">{card.cityLine}</span>
        </span>
