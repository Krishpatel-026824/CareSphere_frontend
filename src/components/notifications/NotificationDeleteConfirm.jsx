...
export default function NotificationDeleteConfirm({ item, onCancel, onConfirm }) {
  if (!item) return null
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy/40">
      ...
      >
        ...
        <h2 id="notification-delete-title" className="text-lg font-bold text-navy mt-4">
          Delete this notification?
        </h2>
        <p className="text-sm text-body-gray mt-2 leading-relaxed">
          <span className="font-semibold text-navy">{item.title}</span> will be removed from your inbox.
