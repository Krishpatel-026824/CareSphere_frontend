      ...
      <DialogContent sx={{ p: 3, position: 'relative' }}>
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between gap-2 mb-2">
            <p className="text-xs font-semibold text-navy">Choose from pharmacy</p>
            <p className="text-[11px] text-body-gray">
              {filteredPharmacy.length} of {pharmacyOptions.length}
            </p>
          </div>
          <div className="rounded-xl border border-border-gray bg-white px-3 py-2 flex items-center gap-2 mb-2">
            ...
            {query ? (
              <button
                type="button"
                onClick={() => setQuery('')}
                className="p-0.5 rounded-md text-body-gray hover:text-navy hover:bg-[#F1F5F9] cursor-pointer"
          ...
          <div className="max-h-52 overflow-y-auto overscroll-contain grid grid-cols-1 sm:grid-cols-2 gap-2 pr-0.5">
            {filteredPharmacy.length === 0 ? (
              <p className="sm:col-span-2 rounded-xl border border-dashed border-border-gray bg-[#F8FAFC] px-3 py-5 text-center text-xs text-body-gray">
                No medicines match “{query.trim()}”. Try another search.
              </p>
            ) : (
              filteredPharmacy.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center gap-2.5 rounded-xl border p-2 text-left cursor-pointer transition-colors ${
                      selected
                        ? 'border-teal bg-teal-light/40 ring-1 ring-teal/30'
                        : 'border-border-gray bg-white hover:border-teal/40 hover:bg-[#F8FBFC]'
                    }`}
                  >
                    ...
                    {selected ? (
                      <span className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full bg-teal text-white inline-flex items-center justify-center">
