...
export default function ProfileHeader({
  ...
  isEditing,
  title = 'Profile',
  subtitle = 'Manage your account and care preferences',
}) {
  return (
    <header className="shrink-0 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
      <div className="flex flex-col gap-1.5 min-w-0">
        ...
        <p className="text-sm text-body-gray leading-snug">{subtitle}</p>
      </div>
      ...
      <button
        ...
        onClick={onEdit}
        disabled={isEditing}
        className="inline-flex items-center justify-center gap-2 self-start sm:self-auto min-h-10 sm:min-h-11 px-4 sm:px-5 rounded-xl bg-teal text-white text-sm font-semibold cursor-pointer hover:bg-teal-dark disabled:opacity-50 disabled:cursor-not-allowed shrink-0 transition-colors shadow-sm"
      >
