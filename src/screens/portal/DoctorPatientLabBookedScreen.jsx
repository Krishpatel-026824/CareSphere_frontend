...
export default function DoctorPatientLabBookedScreen({ patient, patientId, onBack, onBookNew }) {
  ...
  return (
    <div className="w-full h-full min-h-0 bg-[#F4F7FA] flex flex-col overflow-hidden">
      <div className="flex-1 min-h-0 page-pad py-3 sm:py-4 overflow-y-auto scroll-y">
        <div className="flex flex-col gap-3 max-w-[960px] mx-auto w-full">
          ...
          <section className="shrink-0 rounded-2xl border border-[#E6EBF1] bg-white shadow-sm overflow-hidden flex flex-col">
            ...
            {!rows.length ? (
              <div className="px-4 sm:px-5 py-10 flex items-center justify-center bg-[#FAFCFD]">
                <p className="max-w-md rounded-2xl border border-dashed border-[#D0D9E3] bg-white px-6 py-5 text-sm text-body-gray text-center leading-relaxed">
                  No booked lab reports yet. Tap New lab report to order tests for this patient.
                </p>
              </div>
            ) : (
              <>
                <PatientChartTable fit={!tableScroll}>
                  <PatientChartThead>
                    <tr>
